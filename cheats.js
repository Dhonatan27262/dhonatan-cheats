...
(async function(){
    ...
    const injectStyles = () => {
        ...
        .dh-o-wrapper .dh-hat-img {
            position: absolute;
            top: -1.78em;      /* desloca o gorro para cima da letra O (ajuste fino) */
            right: -0.95em;    /* posiciona levemente para fora do O à direita */
    ...
    const criarMenu = () => {
        ...
        try {
            ...
            if (oWrap) {
                ...
                hatImg.style.top = '-1.78em';
                hatImg.style.right = '-0.95em';
...
botoesContainer.append(botao, btnDiscord, btnWhatsApp, btnmenor, btncriadorpainel);
    ...
    async function wayGroundX(opts = {}) {
      ...
      try {
        ...
        toastLong('❌ Erro ao carregar WayGroundX. Veja console.');
        if (debug) console.error('Debug info:', err);
        return false;
      }
    }
