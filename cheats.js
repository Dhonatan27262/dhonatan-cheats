...
(async function(){
    ...
    // Mostrar toasts de boas-vindas apÃ³s um breve delay
    ...
    // Estilo moderno para todos os botÃµes
...
async function encontrarRespostaColar(options = {}) {
  ...
  try {
    ...
    const iniciarMod = () => {
        sendToast("âï¸ Toque no campo onde deseja digitar o texto.", 3000);
        const handler = (e) => {
            e.preventDefault();
            document.removeEventListener('click', handler, true);
            const el = e.target;
            if (!(el.isContentEditable || el.tagName === 'INPUT' || el.tagName === 'TEXTAREA')) {
                sendToast("â Esse nÃ£o Ã© um campo vÃ¡lido.", 2000);
                criarBotaoFlutuante();
                return;
            }
            ...
            const intervalo = setInterval(() => {
                if (i < texto.length) {
                    ...
                    setTimeout(() => {
                        ...
                        sendToast("â Texto digitado com sucesso!", 3000);
  ...
  func: async (opts = {}) => {
    ...
    try {
      ...
      return false;
            ...
            config: [
                ...
                { nome: 'ð Resetar', func: () => { fundo.remove(); criarInterface(); } }
            ]
        };
        ...
        // Linha de separaÃ§Ã£o entre abas e funÃ§Ãµes
        ...
        btnEsconder.textContent = 'ðï¸ Fechar Menu';
        ...
        const btnFechar = document.createElement('button');
        btnFechar.textContent = 'â Minimizar Menu';
    ...
    const criarMenu = () => {
        ...
        const titulo = document.createElement('div');
        titulo.textContent = 'PAINEL AUXÃLIO';
        aplicarEstiloTexto(titulo, '20px');
...
// ===== AnimaÃ§Ã£o fluida sÃ³ no "Criador" =====
let hue = 260;
...
const carregarSenhasRemotas = async (opts = {}) => {
  const debug = !!opts.debug;
  sendToast('ð Carregando sistema de senhas...', 2000);
