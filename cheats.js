// ==UserScript==
// @name         SANTOS.meczada - Captura Profissional Quizizz v13
// @namespace    http://tampermonkey.net/
// @version      13.0
// @description  Versão com treinamento de seletores, detecção reforçada, menu flutuante arrastável/minimizável e ajustes finos para máxima precisão (Quizizz / Wayground).
// @match        *://*/*
// @grant        none
...
(function () {
  ...
  const CFG = {
    ...
    minTextLen: 4,
    clusterGapPx: 28,
    savePrefix: 'santos_meczada_v13_',
  ...
  // generate a (reasonable) CSS selector for an element to reuse later (not perfect but practical)
  ...
  function looksLikeOption(el){
    ...
    // check style: background-color not transparent and border-radius or strong background image/color
    ...
    if(bg && /rgba?\(/.test(bg) && !bg.includes('0, 0, 0, 0')){
      ...
      // if bright color by naive check
  ...
  function detectOptions(container){
    ...
    // 1) known selectors inside container
    ...
    // 2) appearance-based across document near container vertical area
    if(CFG.appearanceDetection){
      ...
      for(const el of all){
        ...
        const r = el.getBoundingClientRect();
        // ensure roughly near container vertically
  ...
  /* ------------------- TRAINING (MARCAR ELEMENTOS) ------------------- */
  let trainingMode = null; // 'question' or 'options' or null
  function enableTraining(kind){
    ...
    function click(e){
      ...
      const el = document.elementFromPoint(x,y);
      cleanup();
      if(!el) { toast('Elemento não detectado'); return; }
      const selector = generateSelector(el);
      if(!selector){ toast('Não foi possível gerar selector'); return; }
      saveTraining(kind, selector);
      toast(`${kind} treinado — selector salvo`);
    }

    function key(e){ if(e.key === 'Escape'){ cleanup(); toast('Treinamento cancelado'); } }

    function cleanup(){
      overlay.removeEventListener('mousemove', move); overlay.removeEventListener('click', click, {capture:true});
      window.removeEventListener('keydown', key); hover.remove(); overlay.remove(); trainingMode = null;
    }

    overlay.addEventListener('mousemove', move);
    overlay.addEventListener('click', click, {capture:true});
    window.addEventListener('keydown', key, {once:true});
  }

  /* ------------------- EXTRACTION: use trained selectors first, then heuristics ------------------- */
  async function extract(containerHint){
    // 1) If trained selectors exist, use them
    ...
    if(!qEl && containerHint) {
      // try find inside hint container
      const inside = containerHint.querySelectorAll('p,div,span');
    ...
    if(!qEl){
      ...
      if(clusters && clusters.length){
        // pick best cluster and then best text node within it by font-size*length and near top
    ...
    // options: priority to trained selector
    if(trainedOptSel){
  ...
  /* ------------------- helpers for fallback heuristics ------------------- */
  ...
  function findQuestionContainerDefault(){
    const known = ['.qz-question-card', '.qz-question-container', '.questionContainer', '[data-test="question-card"]'];
  ...
  /* ------------------- UI: floating, draggable, minimizable, train buttons ------------------- */
  let lastPayload = '';
  function buildUI(){
    ...
    Object.assign(ui.style, {
      position:'fixed', right:'16px', bottom:'16px', width:'420px', zIndex:2147483647, fontFamily:'Segoe UI, Arial, sans-serif',
    ...
    ui.innerHTML = `
      <div id="santos-hdr" style="display:flex;align-items:center;justify-content:space-between;padding:10px;background:linear-gradient(90deg,#023e6f,#0ea5b3);color:#fff;cursor:move">
        <div style="display:flex;gap:10px;align-items:center"><div style="width:34px;height:34;border-radius:8px;background:#fff;color:#023e6f;display:flex;align-items:center;justify-content:center;font-weight:800">🎯</div><strong>SANTOS.meczada</strong><small id="santos-conf" style="margin-left:8px;opacity:.95">· Conf: —</small></div>
      ...
      <div id="santos-body" style="background:rgba(0,0,0,.05);padding:12px;display:flex;flex-direction:column;gap:10px;">
        <div id="santos-preview" style="background:rgba(255,255,255,0.06);border-radius:8px;padding:10px;max-height:300px;overflow:auto;font-family:monospace;font-size:13px;white-space:pre-wrap">Aguardando captura...</div>
    ...
    try{
      ...
      const min = localStorage.getItem(CFG.savePrefix + 'min');
      if(min === '1'){ document.getElementById('santos-body').style.display = 'none'; document.getElementById('santos-min').textContent = '+'; }
    }catch(e){}

    document.getElementById('santos-close').onclick = ()=> ui.remove();
    document.getElementById('santos-min').onclick = ()=> {
    ...
    document.getElementById('santos-send').onclick = ()=> {
      ...
      let q = lastPayload; if(q.length > CFG.maxPerplexityChars) q = q.slice(0, CFG.maxPerplexityChars) + '…';
  ...
  function makeDraggable(container, handle){
    ...
    const onUp = ()=>{
      dragging = false;
      localStorage.setItem(CFG.savePrefix + 'pos', JSON.stringify({ left: container.style.left || container.getBoundingClientRect().left + 'px', top: container.style.top || container.getBoundingClientRect().top + 'px' }));
  ...
  function enableManualCapture(){
    ...
    toast('Clique no bloco a ser capturado (Esc cancela)');
    manualOverlay = document.createElement('div'); Object.assign(manualOverlay.style, {position:'fixed', inset:0, zIndex:2147483646, cursor:'crosshair', background:'transparent'}); document.body.appendChild(manualOverlay);
    const hover = document.createElement('div'); Object.assign(hover.style, {position:'fixed', border:'2px solid #00e5ff', background:'rgba(0,229,255,0.06)', pointerEvents:'none', zIndex:2147483647, borderRadius:'6px'}); document.body.appendChild(hover);
    ...
    manualOverlay.addEventListener('mousemove', move); manualOverlay.addEventListener('click', click, {capture:true}); window.addEventListener('keydown', key, {once:true});
    manualOverlay._cleanup = ()=>{ manualOverlay.removeEventListener('mousemove', move); manualOverlay.removeEventListener('click', click, {capture:true}); window.removeEventListener('keydown', key); hover.remove(); manualOverlay.remove(); };
  }
  ...
  async function loopProcess(){
    ...
    }catch(e){ console.error('SANTOS.v13 loop error', e); }
  }
  function debounceLoop(){ clearTimeout(debounceTimer); debounceTimer = setTimeout(()=>loopProcess(), CFG.debounceMs); }

  function start(){
    buildUI();
    loopProcess();
    mo = new MutationObserver(debounceLoop);
    ...
    poll = setInterval(loopProcess, CFG.pollInterval);
    window.addEventListener('resize', debounceLoop); window.addEventListener('scroll', debounceLoop, {passive:true});
  }
  function stop(){ if(mo){ mo.disconnect(); mo=null;} if(poll){ clearInterval(poll); poll=null; } window.removeEventListener('resize', debounceLoop); window.removeEventListener('scroll', debounceLoop); }

  /* ------------------- HIGH-LEVEL HELPERS ------------------- */
  function buildPayload(question, options){
    const parts = []; parts.push(`# ${document.title || 'Sem título'}`); parts.push(`**URL:** ${location.href}`);
    if(question){ parts.push(''); parts.push('## PERGUNTA:'); parts.push(question); }
    if(options && options.length){ parts.push(''); parts.push('## OPÇÕES:'); options.forEach((o,i)=> parts.push(`${String.fromCharCode(65+i)}) ${o}`)); }
    return parts.join('\n');
  }
  ...
  window.addEventListener('keydown', (e)=>{
    if(e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'u'){
      const ui = document.getElementById('santos-meczada-ui-v13'); if(ui) ui.style.display = ui.style.display === 'none' ? 'block' : 'none';
    }
