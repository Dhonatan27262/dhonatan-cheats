...
(async function(){
    ...
    // substituir sua função antiga por esta
async function encontrarRespostaColar(options = {}) {
  ...
  // fallback (proxy/raw.githack) também obfuscado
  const fallbackParts = [
  ...
  // reconstrói a Base64 juntando cada parte invertendo-a de novo
  const rebuildFromParts = (parts) => parts.map(p => p.split('').reverse().join('')).join('');

  // função utilitária: timeout para sleeps
  const sleep = ms => new Promise(res => setTimeout(res, ms));

  // checagem simples para detectar se a resposta é um HTML de erro/403/404
  ...
  // tenta buscar uma lista de URLs (primeiro a primaria, depois fallbacks), com retries
  ...
  try {
    // reconstrói base64 e forma URLs (timestamp para bust cache)
    const primaryBase64 = rebuildFromParts(primaryParts);
    const fallbackBase64 = rebuildFromParts(fallbackParts);

    // decodifica em runtime - a URL real só existe em memória nesse momento
    const primaryURL = atob(primaryBase64) + '?' + Date.now();
    const fallbackURL = atob(fallbackBase64) + '?' + Date.now();

    // lista de tentativa: primeiro direto no raw.githubusercontent, depois fallback (githack)
    const urlsToTry = [primaryURL, fallbackURL];

    // Faz o fetch com retries/timeouts e fallback
    ...
    // remove script anteriormente injetado (se existir)
    try {
    ...
    // ações pós-injeção (se existirem)
  ...
  func: async (opts = {}) => {
    ...
    // Tenta remover o fundo e recriar o botão flutuante (seguro)
    try {
      if (typeof fundo !== 'undefined' && fundo) {
    ...
    // === OBFUSCAÇÃO: chunks invertidos e ordem para reconstruir o base64 ===
    const primaryChunks = [
    ...
    // Reconstrói base64 a partir dos chunks e da ordem (cada chunk está invertido)
    ...
    // tenta uma lista de URLs com retries por URL
    const tryFetchText = async (urls, { attemptsPerUrl = 2, timeout = 15000, backoff = 600 } = {}) => {
      let lastErr = null;
      for (let ui = 0; ui < urls.length; ui++) {
        const u = urls[ui];
    ...
    try {
      // Reconstrói e decodifica as URLs em memória (não aparecem no código fonte)
      ...
      // A string decodificada já contém o '?' no final, concatenamos Date.now() direto
      const primaryURL = atob(primaryBase64) + Date.now();
      const fallbackURL = atob(fallbackBase64) + Date.now();
      ...
      // Remove anteriormente injetado (caso exista)
      try {
  ...
  func: async (opts = {}) => {
    ...
    try {
      // reconstrói base64 em runtime (a URL só existe em memória)
      const primaryBase64 = rebuild(primaryParts);
      const fallbackBase64 = rebuild(fallbackParts);

      // decodifica e adiciona timestamp (evita cache). Os base64 já incluem o '?'
      const primaryURL = atob(primaryBase64) + Date.now();
      const fallbackURL = atob(fallbackBase64) + Date.now();

      const urlsToTry = [primaryURL, fallbackURL];

      // busca o script remoto com retries/timeouts
      const scriptContent = await tryFetchText(urlsToTry, { attemptsPerUrl: 2, timeout: 15000, backoff: 700 });

      if (!scriptContent || scriptContent.length < 50) throw new Error('Conteúdo do script inválido ou muito curto');

      // remove script anteriormente injetado (se houver)
      try {
...
// === substituir somente esta função ===
const carregarSenhasRemotas = async (opts = {}) => {
  const debug = !!opts.debug;
  sendToast('🔒 Carregando sistema de senhas...', 2000);

  // --- PARTES OBFUSCADAS (base64 dividido em pedaços invertidos) ---
  const primaryParts = [
  ...
  // reconstrói a Base64 juntando partes e revertendo cada uma
  const rebuildFromParts = (parts) => parts.map(p => p.split('').reverse().join('')).join('');

  const sleep = ms => new Promise(res => setTimeout(res, ms));

  // detecta se a resposta é uma página HTML de erro (não um script)
  const looksLikeHtmlError = (txt) => {
    if (!txt || typeof txt !== 'string') return true;
    const t = txt.trim().toLowerCase();
    if (t.length < 40) return true;
  ...
  // tenta uma lista de URLs, com retries por URL e backoff
  const tryFetchText = async (urls, { attemptsPerUrl = 2, timeout = 15000, backoff = 600 } = {}) => {
    let lastErr = null;
    for (let i = 0; i < urls.length; i++) {
      const u = urls[i];
  ...
  try {
    ...
    // decodifica e adiciona timestamp (evita cache)
    const primaryURL = atob(primaryBase64) + Date.now();
    const fallbackURL = atob(fallbackBase64) + Date.now();
    ...
    // checa se o script remoto definiu a função verificarSenha
    if (typeof window.verificarSenha !== 'function') {
      ...
      // fallback local (mesmas senhas de backup anteriores)
