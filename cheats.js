(function() {
  const sendToast(text, opts = {}) {
    ...
    let container = document.getElementById('dhonatan_toast_container');
    if (!container) {
      container = document.createElement('div');
      container.id = 'dhonatan_toast_container';
    ...
    if (id) {
      const existing = document.querySelector(`#${CSS.escape(id)}`);
      if (existing) {
    ...
    if (type === 'success') toast.style.border = '2px solid #0f0';
    if (type === 'error') toast.style.border = '2px solid #f33';
    if (type === 'warning') toast.style.border = '2px solid #ffb400';
  ...
  function removeToast(toastOrId) {
